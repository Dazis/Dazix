    ...
    $.fn.jVerify = function( opts ){
        ...
        this.on('focus', function(){
            ...
            if( /\s/.test(self.val()) && !_opt.spaceOk ){
                _opt.warnElement && _opt.warnElement.html( '<em class="error"></em>' + _opt.spaceTips ).css('color', '#f00');
            ...
            if( val == "" ){
                ...
                if( _opt.type !== 'psw' && _opt.type !== 'confirmpsw' ){
                    _opt.warnElement && _opt.warnElement.html( '<em class="success"></em>&nbsp;' ).css('color', '0c0');
                }
            }else if( _checkVal === 4 ){

            }else{
                _opt.warnElement && _opt.warnElement.html( '<em class="error"></em>' + _opt.warnMsg ).css('color', 'f00');
            }
        });
        this.on(_opt.vEvent, function(e){
            if($(this).attr('data-times')){
                checkValue(e);
            }else{
            ...
            switch (result){
                case 0:
                    if( _opt.emptyOk ){
                        ...
                        _opt.warnElement && _opt.warnElement.html( '<em class="error"></em>' + _opt.emptyMsg ).css('color', '#f00');
                        self.data('status', false);
                    }else{
                        //alert( _opt.emptyMsg );
                    }
                ...
                case 2:
                    if( _opt.warnElement ){
                        _opt.warnElement.html( '<em class="error"></em>' + _opt.warnMsg ).css('color', '#f00');
                    }else{
                        //alert( _opt.warnMsg );
                    }
                    self.data('status', false);
                ...
                case 100:
                    if( _opt.warnElement && _opt.type !== 'psw' ){
                        _opt.warnElement.html('<em class="success"></em>&nbsp;').css('color', '#0c0');
                    }else{
        ...
        function _check(type, str){
            ...
            switch (type){
                ...
                case 'int':
                    ...
                    return 100;
                case 'mobile':
                    if(!/^1[345678][0-9]{9}$/.test(str)) return 3;
                    return 100;
                    break;
                case 'email':
                    if(_opt.minLength && str.length < _opt.minLength) return 1;
